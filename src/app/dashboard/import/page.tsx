            ...
            <div key={idx} className="space-y-3">
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  ...
                  <div className="pt-2 border-t border-[#E8EDE9] flex items-center justify-end gap-2 text-sm font-medium text-[#2D3B35]">
                    <span>Total</span>
                    <div className="flex items-center gap-1">
                      <span className="text-[#8A9A8E]">$</span>
                      <Input
                        type="number" step="0.01" min="0"
                        className="h-7 w-28 text-right text-sm font-semibold"
                        value={order.total_override ?? order.items.reduce((s, it) => s + it.quantity * it.unit_price, 0).toFixed(2)}
                        placeholder="0.00"
                        onChange={(e) => updateOrder(idx, o => ({ ...o, total_override: e.target.value }))}
                      />
                    </div>
                    {order.total_override !== undefined && (
                      <button
                        onClick={() => updateOrder(idx, o => { const { total_override: _, ...rest } = o; return rest as ParsedOrder; })}
                        className="text-[10px] text-[#8A9A8E] hover:text-[#D97B6C] hover:underline"
                      >
                        reset
                      </button>
                    )}
